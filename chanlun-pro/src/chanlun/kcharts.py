...
def zss_to_charts(zss: List[ZS]):
    ...
    for zs in zss:
        ...
        # 两竖，两横，5 个点，转一圈，之后是中枢级别，是否完成中枢，是否有效
        # 第 5 个元素：有效性标记（True=正确，False=不正确）
        # 第 6 个元素：标记文本（"正确"/"不正确"）
        is_valid = getattr(zs, '_is_valid', True)  # 默认 True 兼容旧代码
        valid_text = "正确" if is_valid else "不正确"  # 矩形内显示的文字
        l_zs = [
            [start_index, start_index, end_index, end_index, start_index],
            [zs.zg, zs.zd, zs.zd, zs.zg, zs.zg],
            zs.level + 1,
            zs.done,
            is_valid,  # 新增：离开笔验证结果（布尔值）
            valid_text,  # 新增：矩形内显示的文字标记
        ]
