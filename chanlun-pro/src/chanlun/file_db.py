...
class FileCacheDB(object):
    ...
    def get_web_cl_data(
        ...
        """
        获取web缓存的的缠论数据对象
        cl_engine: 'cl' 加密版, 'cl1' 开源版
        """
        _cl_mod = cl
        if cl_engine == 'cl1':
            from chanlun import cl1 as _cl_mod
        elif cl_engine == 'cl2':
            from chanlun import cl2 as _cl_mod
        elif cl_engine == 'cl3':
            from chanlun import cl3 as _cl_mod
        ...
        file_pathname = (
            self.cl_data_path
            / market
            / f"{market}_{code.replace('/', '_').replace('.', '_')}_{frequency}_{key}_{cl_engine}.pkl"
        )
        cd: ICL = _cl_mod.CL(code, frequency, cl_config)
        try:
            if file_pathname.is_file():
                ...
                if (
                    ...
                    # )
                    cd = _cl_mod.CL(code, frequency, cl_config)
