...
for idx, zs in enumerate(zss):
    fn = idx + 1
    print(f"ZS{fn} type={zs.type} lines={zs.line_num} b[{zs.lines[0].index}]~b[{zs.lines[-1].index}] "
          f"ZG={zs.zg:.2f} ZD={zs.zd:.2f}")
    
# 详细显示 ZS15 周围
print("\n=== ZS15 周围笔 (b[515]~b[540]) ===")
for bi_idx in range(515, 541):
    b = bis[bi_idx]
    print(f"  b[{bi_idx}] {b.type:5s} start={b.start.val:8.2f} end={b.end.val:8.2f} "
          f"time={b.start.k.date}~{b.end.k.date}")
