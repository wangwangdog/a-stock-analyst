...
def fetch_kline_cross_checked(symbol: str, start_date: str = None, end_date: str = None) -> dict:
    ...
    if not cache_bs.empty:
        ...
        result["source"] = "cache"
        result["status"] = "ok"
        result["message"] = "使用缓存数据 (baostock)"
        return result
    if not cache_ak.empty:
        result["primary"] = cache_ak
        result["source"] = "cache"
        result["status"] = "ok"
        result["message"] = "使用缓存数据 (akshare)"
        return result

    # 1.5 TDX 缓存兜底（在尝试网络请求之前，避免AKShare超时）
    # 先查 period='daily'，再查 period='d'（旧格式）
    cache_tdx = get_kline(symbol, "tdx", start_date, end_date, period="daily")
    if cache_tdx.empty:
        cache_tdx = get_kline(symbol, "tdx", start_date, end_date, period="d")
    if not cache_tdx.empty:
        result["primary"] = cache_tdx
        result["source"] = "cache"
        result["status"] = "ok"
        result["message"] = "使用缓存数据 (tdx)"
        return result
    ...
    # 4. TDX 缓存兜底（指数等AKShare/Baostock不支持的数据源）
    cache_tdx = get_kline(symbol, "tdx", start_date, end_date)
    if not cache_tdx.empty:
        result["primary"] = cache_tdx
        result["source"] = "cache"
        result["status"] = "ok"
