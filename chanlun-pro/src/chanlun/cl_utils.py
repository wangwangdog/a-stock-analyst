...
def web_batch_get_cl_datas(
    ...
    WEB端批量计算并获取 缠论 数据
    内部使用文件缓存，只能进行增量更新
    :param cl_engine: 'cl' 加密版, 'cl1' 开源版
    """
    cls = []
    fdb = FileCacheDB()
    for f, k in klines.items():
        cls.append(fdb.get_web_cl_data(market, code, f, cl_config, k, cl_engine=cl_engine))
    return cls
...
def cl_data_to_tv_chart(
            ...
            fx_data.append(
                {
                    "points": [
                        {"time": ts, "price": pr},
                        {"time": ts + 900, "price": pr + max(pr * 0.0005, 0.0025)},
                    ],
                    "text": fx.type,
                }
            )
    ...
    return {
        ...
        "bcs": bc_chart_data,
        "mmds": mmd_chart_data,
        "trends": _build_trend_chart_data(cd),
    }


def _build_trend_chart_data(cd) -> list:
    """生成走势类型标识数据（中文提示）"""
    try:
        trend_infos = cd.get_trend_infos() if hasattr(cd, "get_trend_infos") else []
    except Exception:
        return []
    ...
    trend_chart_data = []
    for info in trend_infos:
        zs_names = info.get("zs_names", "")
        trend_text = info["trend_type"]
        if zs_names:
            label = f"{trend_text} ({zs_names})"
        else:
            label = trend_text

        # 在走势类型结束位置标识（es_end控制：默认用结束点，也可用起点）
        label_time = info.get("end_time") or info.get("start_time")
        label_price = info.get("end_price") or info.get("start_price")
        if label_time:
            import datetime as _dt
            if hasattr(label_time, "timestamp"):
                ts = int(label_time.timestamp())
            else:
                ts = int(_dt.datetime.timestamp(label_time)) if isinstance(label_time, _dt.datetime) else 0

            trend_chart_data.append({
                "text": label,
                "points": {"time": ts, "price": label_price or 0},
