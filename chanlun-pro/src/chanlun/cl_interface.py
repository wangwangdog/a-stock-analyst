...
class ZS:
    ...
    def __init__(
        ...
        self.exit_line = None  # 离开笔（离开中枢的笔）
...
class BI(LINE):
    ...
    def get_mmds(self, zs_type: str = None) -> List[MMD]:
        # 返回买卖点，需要检查买点的中枢是否有效
        if zs_type is None:
            # 聚合所有 zs_type 下的 MMD + self.mmds
            all_mmds = list(self.mmds)
            seen = set(id(m) for m in all_mmds)
            for mmds in self.zs_type_mmds.values():
                for m in mmds:
                    if id(m) not in seen:
                        all_mmds.append(m)
                        seen.add(id(m))
            return [_m for _m in all_mmds if _m.zs.real]
