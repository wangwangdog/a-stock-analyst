...
with ThreadPoolExecutor(max_workers=3) as pool:
    futures = {pool.submit(fetch, s): s for s in stale}
...
el = time.time() - t0
cnt = sqlite3.connect(DB).execute("SELECT COUNT(*) FROM kline_cache WHERE period='5m' AND trade_date>='2026-05-28'").fetchone()[0]
print(f"完成! {done}只 +{total_rows}行 {el:.0f}s | 5m>=05-28共{cnt}行", flush=True)
