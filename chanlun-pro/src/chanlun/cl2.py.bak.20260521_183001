...
def _build_zss_from_bis(bis: List[BI], zs_type: str = "bi", config: dict = None) -> List[ZS]:
    ...
    步骤：
    1. 连续三笔重叠区间检查 (SHOW_BIS_OVERLAP_CHECK=1)
    2. 进入笔起点检查：起点不能在中枢内 (SHOW_BIS_FALLBACK=1)
    3. 5笔模型高度检查 + 延伸突破 (SHOW_BIS_EXTEND_BREAK=1)
    4. 延伸扩展 (SHOW_BIS_EXTEND=1, config:zs_extend)
    5. 最终 ZG/ZD/GG/DD 重算 (SHOW_BIS_FINAL_RECALC=1)
    6. 中枢合并 (SHOW_BIS_MERGE=1)
    ...
    while i < len(bis) - 2:
        ...
        if zg > zd:
            ...
            if SHOW_BIS_EXTEND_BREAK and i + 4 < len(bis):
                ...
                # ── 5笔高度检查：while条件自然判断 ──
                # up: exit_end < entry_end→继续延伸; exit_end > entry_end→结束
                # down: exit_end > entry_end→继续延伸; exit_end < entry_end→结束
                while ((zs_direction == "up" and exit_end < entry_end) or \
                       (zs_direction == "down" and exit_end > entry_end)) \
                    ...
                    if bh >= zd and bl <= zg:
                        ...
                        sub = zs_lines[1:-1]
                        if sub:
                            zg = min(b.high for b in sub)
                            zd = max(b.low for b in sub)
