...
# 股票涨跌幅用 kline_cache（与K线图同源）
_ticks_cache_conn = None


def _get_ticks_cache_conn():
    global _ticks_cache_conn
    if _ticks_cache_conn is None:
        import sqlite3
...
def create_app(test_config=None):
    ...
    def tv_symbols():
        ...
        # A股 code 需要补全市场前缀 (000001 -> SZ.000001, 600000 -> SH.600000)
        if market == "a" and not code.startswith("SZ.") and not code.startswith("SH.") and not code.startswith("BJ."):
        ...
        if stocks is None:
            return {"error": f"stock {code} not found"}, 404
    ...
    def tv_history():
        ...
        cl_engine = request.args.get("cl_engine", request.cookies.get("cl_engine", "cl2"))
        ...
        if (
            ...
            # __pxy_log.info(f'{code} - {frequency_low} enable low to high get klines time : {time.time() - s_time}')
            # s_time = time.time()
            cd = web_batch_get_cl_datas(
                market, code, {frequency_low: klines}, cl_config, cl_engine=cl_engine
            )[0]
            # __pxy_log.info(f'{code} - {frequency_low} enable low to high get cd time : {time.time() - s_time}')
        else:
            kchart_to_frequency = None
            # s_time = time.time()
            klines = ex.klines(code, frequency)
            # __pxy_log.info(f'{code} - {frequency} get klines time : {time.time() - s_time}')
            # s_time = time.time()
            cd = web_batch_get_cl_datas(market, code, {frequency: klines}, cl_config, cl_engine=cl_engine)[0]
            # __pxy_log.info(f'{code} - {frequency} get cd time : {time.time() - s_time}')
        ...
        # __pxy_log.info(f'{code} - {frequency} to tv chart data time : {time.time() - s_time}')
        ...
        if firstDataRequest == "false":
            ...
            _trends = cl_chart_data.get("trends", [])
    ...
    def tv_time():
        ...
        服务器时间
        """
        return str(fun.datetime_to_int(datetime.datetime.now()))
    ...
    def ticks():
        ...
        # 统一从 kline_cache 获取最新日线收盘价
        res_ticks = []
        try:
            conn = _get_ticks_cache_conn()
            cur = conn.cursor()
            for _c in codes:
                # 先查完整代码带前缀（如 SH.000001），再查去前缀的裸代码（如 000001）
                query_keys = [_c]
                bare = _c.split(".")[-1] if "." in _c else _c
                if bare != _c:
                    query_keys.append(bare)
                found = None
                for qk in query_keys:
                    cur.execute(
                        "SELECT close, trade_date FROM kline_cache WHERE symbol=? AND period='daily' ORDER BY trade_date DESC LIMIT 1",
                        (qk,),
                    )
                    row = cur.fetchone()
                    if row is not None:
                        found = (qk, row[0], row[1])
                        break
                if found is not None:
                    qk, close, trade_date = found
                    # 前日收盘价也用同一key查询（保证同一条数据源）
                    cur.execute(
                        "SELECT close FROM kline_cache WHERE symbol=? AND period='daily' AND trade_date<? ORDER BY trade_date DESC LIMIT 1",
                        (qk, trade_date),
                    )
                    prev_row = cur.fetchone()
                    prev_close = prev_row[0] if prev_row else close
                    rate = round((close - prev_close) / prev_close * 100, 2) if prev_close else 0.0
                    res_ticks.append({"code": _c, "price": close, "rate": rate})
        except Exception:
            traceback.print_exc()
        return {"now_trading": False, "ticks": res_ticks}
    ...
    # ── A-Stock Backend Proxy (FastAPI :9901) ──
    import logging as _lg
    _proxy_log = _lg.getLogger('a-stock-proxy')

    def _proxy_to_fastapi(path: str):
        """转发请求到 FastAPI :9901"""
        import httpx as _httpx
        target = f"http://127.0.0.1:9901/{path}"
        qs = request.query_string.decode()
        if qs:
            target += f"?{qs}"
        _proxy_log.info(f"[{request.method}] /{path}")
        try:
            with _httpx.Client(timeout=60.0) as client:
                ...
                resp = client.request(
                    ...
                    headers={"Content-Type": ct}, content=body,
                )
                return (resp.content, resp.status_code, dict(resp.headers))
        except _httpx.ConnectError:
            return {"error": "a-stock-backend 未启动"}, 502

    @app.route("/backend/<path:subpath>", methods=["GET", "POST", "PUT", "DELETE", "OPTIONS"])
    @app.route("/backend", methods=["GET", "POST", "PUT", "DELETE", "OPTIONS"])
    def backend_proxy(subpath=""):
        return _proxy_to_fastapi(subpath)

    # ── RSS 新闻 API（独立前缀，不经过 /api/ proxy）──
    import json as _json
    from .rss_fetcher import get_news, get_news_count, get_sources, fetch_all, get_dedup_stats, init_tables as _rss_init
    _rss_init()

    @app.route("/rss-api/sources")
    def rss_sources():
        return {"sources": get_sources(), "total": get_news_count()}

    @app.route("/rss-api/list")
    def rss_list():
        limit = request.args.get("limit", 50, type=int)
        offset = request.args.get("offset", 0, type=int)
        source = request.args.get("source", None)
        return {"news": get_news(limit=limit, offset=offset, source=source)}

    @app.route("/rss-api/stats")
    def rss_stats():
        return get_dedup_stats()

    @app.route("/rss-api/fetch", methods=["POST"])
    def rss_fetch():
        try:
            total_raw, stats = fetch_all()
            return {"ok": True, "raw_added": total_raw, "deduped_added": stats["deduped"], "skipped_url": stats["skipped_url"], "skipped_title_exact": stats["skipped_title_exact"], "skipped_title_sim": stats["skipped_title_sim"]}
        except Exception as e:
    ...
    def api_proxy(subpath=""):
        return _proxy_to_fastapi(f"api/{subpath}")

    # ── A-Stock Frontend SPA (Vue3) ──
    _a_stock_dir = pathlib.Path(__file__).resolve().parent / "static" / "a-stock"
    _a_stock_index = str(_a_stock_dir / "index.html")

    @app.route("/a-stock/")
    @app.route("/a-stock/<path:subpath>")
    def a_stock_spa(subpath=""):
        fp = _a_stock_dir / subpath if subpath else None
        if fp and fp.is_file():
            return send_file(str(fp))
        return send_file(_a_stock_index)
