...
def _build_zss_from_bis(bis: List[BI], zs_type: str = "bi", config: dict = None) -> List[ZS]:
    ...
    while i < len(bis) - 2:
        ...
        if zg > zd:
            ...
            while extend_check and config.get('zs_extend', 1) and j < len(bis):
                bh, bl = bis[j].high, bis[j].low
                if bh >= zd and bl <= zg:  # 与当前中间笔区间有交集即可延伸
    ...
    while merged:
        ...
        for k in range(len(zss)):
            ...
            if k + 1 < len(zss):
                ...
                if (shared or adj) and (zss[k].gg > zss[k + 1].dd and zss[k].dd < zss[k + 1].gg):
                    a, b = zss[k], zss[k + 1]
                    # 去重合并 lines
                    seen = set()
...
def _build_zss_with_higher_level(bis: List[BI], higher_direction: str, zs_type: str = "bi",
    ...
    while i < len(bis) - 2:
        ...
        if zg > zd:
            ...
            zg = max(fractal_prices)
            zd = min(fractal_prices)
            gg = zg
            dd = zd
            zs_lines = [b1, b2, b3]
            j = i + 3
            while config.get('zs_extend', 1) and j < len(bis):
                # 扩展笔也必须在高一级别边界内
                if higher_bis:
